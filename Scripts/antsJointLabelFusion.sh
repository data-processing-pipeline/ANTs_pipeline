...
PEXEC=${ANTSPATH1}ANTSpexec.sh
SGE=${ANTSPATH1}waitForSGEQJobs.pl
PBS=${ANTSPATH1}waitForPBSQJobs.pl
XGRID=${ANTSPATH1}waitForXGridJobs.pl
SLURM=${ANTSPATH1}/waitForSlurmJobs.pl
...
for (( i = 0; i < ${#ATLAS_IMAGES[@]}; i++ ))
  do
    ...
    regcall=${ANTSPATH1}/antsRegistrationSyN.sh
    if [[ $RUNQUICK -eq 1 ]];
      then
        regcall=${ANTSPATH1}/antsRegistrationSyNQuick.sh
      fi
...
if [[ $DOQSUB -eq 1 ]];
  then
    ...
    ${ANTSPATH1}/waitForSGEQJobs.pl 1 600 $jobIDs
  fi
if [[ $DOQSUB -eq 4 ]];
  then
